...
import matplotlib.pyplot as plt # Gráficoss
...
def distancias(file_name, pontos): # Calcula as distâncias dentro da gama especificada
    dist = []
    j = 0
    ...
    for index, (timestamp, decoded_message) in enumerate(log.parser()):
        ...
        if(600<=index<=1000):
            dist.append(meters_per_sample(decoded_message) * (pontos[index-600]+600+0.5))
...
def meters_per_sample(message,  v_som = 1500): # Cálculo da resolução
...
f = open('teste0446.xlsx', 'r')
df = pd.read_excel('teste0446.xlsx')
#print(len(df))
#print(df[0][0])
#print(df)
...
for i in range(0,L): # Processamento - encontrar os pontos de "referência"
    max = 100
    max_pos = 0
    for j in range(5,N-5):
        if(df[j][i] >= max):
            ...
            if(count>=5):
                max = df[j][i]
                max_pos = j
    pontos.append(max_pos)

# Calcula as distâncias 
dist = distancias('20220701-143903394.bin', pontos)
print(len(dist))
print(len(pontos))
# print(pontos)
# print(dist)

#for h in range(0,len(dist)-1):  # Tirei isto que é para tirar as amostras 
 #   if (dist[h] < 0.75):
 #       dist[h] = 0.75
...
if(len(ret1) >10):  # Processamento. Caso tenha mais de10 pontos, considera-se que há ali uma reta
    print("Reta 1")
...
if(len(ret1)>10):  # Ou temos interseção entre 1 3 6 8
...
if(len(ret2)>10): # Ou entre 2 4 5 7
...
plt.savefig('teste0446Desenho.png')
